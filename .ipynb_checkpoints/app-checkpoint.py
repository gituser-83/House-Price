...
# CSS Styles for customizing the app's look
st.markdown("""
    <style>
...
# Organizing input fields in 2 columns and 4 rows
...
with col1:
    ...
    AveRooms = st.number_input('Average Number of Rooms per Household', min_value=0)
    AveBedrms = st.number_input('Average Number of Bedrooms per Household', min_value=0)

with col2:
    Population = st.number_input('Block Group Population (in 1K)', min_value=0)
    AveOccup = st.number_input('Average Number of Household Members', min_value=0)
...
if st.button('Predict House Price'):
    # Prepare the input features as a numpy array for the model
    input_features = np.array([[MedInc, HouseAge, AveRooms, AveBedrms, Population, AveOccup, Latitude, Longitude]])
    
    # Get prediction from the model
    prediction = model.predict(input_features)

    # Multiply the prediction by 10,000 (if that's your intended scaling)
